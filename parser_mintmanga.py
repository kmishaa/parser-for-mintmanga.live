...
URL_FIRST = '/list?sortType=updated'
URL_NOT_FIRST = '/list?sortType=DATE_UPDATE&offset='
...
def get_all_tegs(soup): #создание единой строки с перечислением жанров для каждого тайтла
    block = soup.find('p', class_='elementList')
    tegs = block.find_all('a', class_='element-link')
    all_tegs = ''
    for teg in tegs:
        all_tegs += teg.get_text(strip=True) + ', '
    return all_tegs[:len(all_tegs)-2]
